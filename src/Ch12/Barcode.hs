...
import Data.Ix (Ix(..))
import Data.List (foldl', group, sort, sortBy, tails, find)
...
import Ch10.Parse                    -- from chapter 10
...
-- zip (map (distance (scaleToOne ps)) srl) digits
...
findMatch = find (not . null) . map (solve . candidateDigits) . tails
